...
use bevy::{
    asset::LoadState, diagnostic::{FrameTimeDiagnosticsPlugin, LogDiagnosticsPlugin}, prelude::*, render::primitives::{Aabb, Sphere}, scene::InstanceId
};
use bevy_imposter::{material::{Imposter, ImposterData, ImposterMode}, node::{ImposterBakeBundle, ImposterBakeCamera, ImposterBakePlugin}};
...
fn main() {
    App::new()
        ...
        .add_systems(Update, (scene_load_check, update, impost, update_lights))
        .run();
}
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>, mut meshes: ResMut<Assets<Mesh>>, mut mats: ResMut<Assets<StandardMaterial>>) {
...
pub fn impost(
    ...
    if k.just_pressed(KeyCode::KeyI) {
        ...
        commands.spawn(MaterialMeshBundle {
            ...
            transform: Transform::from_translation(Vec3::X * 0.5 + Vec3::from(scene_handle.sphere.center)),
